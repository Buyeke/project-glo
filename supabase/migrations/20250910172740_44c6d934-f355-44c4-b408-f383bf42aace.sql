-- Fix data exposure vulnerabilities by tightening RLS policies on existing tables
...
-- Secure job_applicants table - remove overly broad policies
...
-- Secure donations table - ensure only admins can view donation data
...
DROP POLICY IF EXISTS "Admins only view donation details" ON public.donations;
...
-- Add deletion restrictions for sensitive data
CREATE POLICY "Only admins can delete profiles" ON public.profiles
FOR DELETE 
TO authenticated
USING (is_admin_user());

-- Secure service_requests_tracking (support requests)
...
INSERT INTO public.security_logs (
  ...
  jsonb_build_object(
    ...
    'tables_secured', ARRAY['profiles', 'job_applicants', 'employer_profiles', 'donations', 'contact_submissions', 'service_requests_tracking'],
    'timestamp', NOW()
  )
);